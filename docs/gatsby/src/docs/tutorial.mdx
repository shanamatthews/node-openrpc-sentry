...
For a given purpose, we may only be interested in a small subset of this information. Let's use the command line utility, `jq` to get the recipient of each transaction from this response.

To save us from making more API calls, save the response to a file by copying and pasting from your terminal, or running the command again and saving the output to a file:
...
Note that command piped the response to `jq '.'` 
